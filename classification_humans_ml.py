...
class AI_alg():
    ...
    def metrics(self):
        ...
        f1 = np.mean(f1_score(self.y_test, self.y_pred, average=None))
        recall =  np.mean(recall_score(self.y_test, self.y_pred, average = 'weighted' ))
        precision = np.mean(precision_score(self.y_test, self.y_pred , average = 'micro'))
